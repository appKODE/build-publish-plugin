...
class LinuxShellCommandExecutor(
  ...
  private fun tagOutputLineToTag(line: String): Tag? {
    ...
    // git describe by default outputs *only* closest annotated tag, use this to find out if last one is annotated
    // (also need to suppress errors to /dev/null, because if git describe doesn't find any tags, it fails with fatal error)
