...
* **Breaking:** ClickUp gains a Jira-style multi-account / multi-project registry. A single `apiTokenFile` becomes named `account("name") { apiTokenFile; project("app") { workspaceName; taskIdPrefix } }` entries (ClickUp has no self-hosted host, so an account carries only a token). `automation` selects projects via `targetAccount("name") { projectNames(…) / project("x") { …overrides } }`; `workspaceName` is no longer a flat automation field. Standalone ClickUp tasks gain `--accountName`
* ClickUp `issueResolution { common { fromAccount("name") { projectNames(…) } } }` opt-in block resolves references to ClickUp task names: a known custom-task-id prefix (`APP-123`) routes to its account+workspace (resolved via `custom_task_ids=true&team_id=<id>`); references without a known prefix are attempted as native ids. `taskIdPrefix` must be globally unique across accounts
...
* Clients: `build-publish-novo-client-jira` adds `getIssueSummary` (backed by a new `GET issue/{key}?fields=summary` call + `GetIssueSummaryResponse`); `build-publish-novo-client-clickup` adds `getTaskName` (reusing the existing `GET task/{id}` call), custom-task-id resolution (`getTaskName(taskId, teamId)` sending `custom_task_ids=true&team_id=<id>`), `getTeamId(workspaceName)`, and a `baseUrl` parameter on `ClickUpControllerFactory.build` (defaulting to the public API; a seam for hermetic mock-server tests)
