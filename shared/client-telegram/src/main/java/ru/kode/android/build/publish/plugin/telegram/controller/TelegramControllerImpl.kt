...
internal class TelegramControllerImpl(
     ...
     *
     * @param message The [TelegramMessage] containing text, bots, header, mentions and issue config
     *
     ...
     */
    override fun send(message: TelegramMessage) {
        val messageWithIssuesLinks = formatIssuesHtml(message.text, message.issueSources)
        val htmlMessage = buildHtmlMessage(message.header, message.userMentions, messageWithIssuesLinks)

        val chunks = htmlMessage.chunkedByLines(MESSAGE_MAX_LENGTH)
        chunks.forEach { chunk ->
            sendMessage(chunk, message.bots)
        }
    }
     ...
     * @return The last [TelegramLastMessage] if available, otherwise null.
     */
...
/**
 * Formats the given [message] by replacing issue keys with hyperlinks.
 *
 * Each [IssueSource] contributes its own extraction pattern and link URL prefix, so keys from
 * different projects/hosts are linked to their respective trackers. Sources without a URL prefix are
 * left unlinked.
 *
 * @param message The message to format.
 * @param issueSources The issue sources (pattern + optional URL prefix) to apply.
 * @return The formatted message with issue keys replaced with hyperlinks.
 */
private fun formatIssuesHtml(
    ...
    issueSources.forEach { source ->
        ...
        val urlPrefix = source.urlPrefix
        if (urlPrefix.isNullOrBlank()) return@forEach
        source.numberPattern.toRegex().findAll(message).distinctBy { it.value }.forEach { match ->
            val issueKey = match.value
            val link = "<a href=\"$urlPrefix$issueKey\">$issueKey</a>"
