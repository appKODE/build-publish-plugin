...
internal class ClickUpControllerImpl(
    ...
    override fun getOrCreateCustomFieldId(
        ...
        val team =
            getTeams()
                ...
                ?: throw IllegalStateException(teamNotFoundForCreateMessage(workspaceName))
        val space =
            getSpaces(team.id).firstOrNull()
                ?: throw IllegalStateException(spaceNotFoundForCreateMessage(team, workspaceName))
        val list =
            getListsInSpace(space.id).firstOrNull()
                ?: throw IllegalStateException(listNotFoundForCreateMessage(team, workspaceName))
        val customField =
    ...
    override fun deleteCustomFieldFromList(
        ...
        val team =
            getTeams()
                .firstOrNull { it.name.equals(workspaceName, ignoreCase = true) }
                ?: throw IllegalStateException(teamNotFoundForDeleteMessage(workspaceName))
        val space =
            getSpaces(team.id).firstOrNull()
                ?: throw IllegalStateException(teamNotFoundForDeleteMessage(team, fieldId))
        val list =
            getListsInSpace(space.id).firstOrNull()
                ?: throw IllegalStateException(listNotFoundForDeleteMessage(space, fieldId))
