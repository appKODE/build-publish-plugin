    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        while (attempt < maxRetries) {
            try {
                ...
                lastException = e
                logger.info(sslHandshakeRetryMessage(attempt + 1, maxRetries, delayMillis))
            } catch (e: EOFException) {
                lastException = e
                logger.info(eofHandshakeRetryMessage(attempt + 1, maxRetries, delayMillis))
            } catch (e: IOException) {
                lastException = e
                logger.info(ioExceptionRetryMessage(attempt + 1, maxRetries, delayMillis))
            }
