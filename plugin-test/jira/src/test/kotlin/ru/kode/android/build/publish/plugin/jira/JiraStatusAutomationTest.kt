...
class JiraStatusAutomationTest {
    ...
    fun `jira status automation executes with automation config`() {
        ...
        awaitUntil("status of $expectedIssueKey is $targetAutomationStatusName") {
            jiraController.getIssueStatus(expectedIssueKey)?.name == targetAutomationStatusName
        }
    }
    ...
    fun `jira status automation executes with automation config with multiple tasks, mixed correct and incorrect status`() {
        ...
        val targetAutomationStatusName1 =
            if (beforeAutomationStatusId?.id == todoStatus?.id) {
                inProgressStatus!!.name
            } else {
                todoStatus!!.name
        ...
        awaitUntil("status of $expectedIssueKey1 is $targetAutomationStatusName1") {
            jiraController.getIssueStatus(expectedIssueKey1)?.name == targetAutomationStatusName1
        }
        awaitUntil("status of $expectedIssueKey2 is $targetAutomationStatusName2") {
            jiraController.getIssueStatus(expectedIssueKey2)?.name == targetAutomationStatusName2
        }
    }
    ...
    fun `jira status automation executes with automation config with multiple tasks, all correct status`() {
        ...
        awaitUntil("status of $expectedIssueKey3 is $targetAutomationStatusName") {
            jiraController.getIssueStatus(expectedIssueKey3)?.name == targetAutomationStatusName
        }
        awaitUntil("status of $expectedIssueKey4 is $targetAutomationStatusName") {
            jiraController.getIssueStatus(expectedIssueKey4)?.name == targetAutomationStatusName
        }
    }
