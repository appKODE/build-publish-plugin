...
class TelegramChangelogTest {
    ...
    @Test
    @Throws(IOException::class)
    fun `telegram changelog sending available with changelog config without proxy and custom server, no user mentions`() {
        projectDir.createAndroidProject(
            buildTypes =
                listOf(
                    BuildType("debug"),
    ...
    @Test
    @Throws(IOException::class)
    fun `telegram changelog sending into forum chat with topic available with changelog config without proxy and custom server`() {
        projectDir.createAndroidProject(
            buildTypes =
                listOf(
                    BuildType("debug"),
    ...
    fun `telegram changelog sending available with changelog config with proxy, but without custom server`() {
        ...
        val assembleResult: BuildResult =
            projectDir.runTask(
                givenAssembleTask,
                taskArguments = proxyProps,
            )
        val changelogResult: BuildResult =
            projectDir.runTask(
        ...
        assembleResult.outputShouldContain("Task :app:getLastTagSnapshotRelease")
